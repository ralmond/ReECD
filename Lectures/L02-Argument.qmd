...
* _Prompt_ -- Statement of the goals of the task
...
* _Work Product and Process_ (WPP)
...
A _task feature_ is a variable that describes some aspect of a task prompt or resources.

An _observable variable_ (_indicator_, _observable_) is a variable that describes the
realized work product or process.
...
A variable passes the clarity test if running the evaluator in a particular situation
produces an unambiguous result [@HowardMatheson1981].

_Latent variables_ are variables that don't pass the clarity test.  Their value must 
...
An _observation_ is a realized observable, usually in a particular situation.

A _claim_ is a prediction about a future observation, in a _context_ (set of possible situations).


# Constructs

A _construct_ is a latent variable associated with a collection of related claims.
...
_Russell's Rule_
| For different levels of the construct to be identifiable, there must be at 
| least one different claim associated with each level.  

## Assessments

An _assessment_ is a series of assigned tasks, whose observation will be used to assign a
value to a construct.

Generally, the _context_ in an assessment is constrained.

![Construct Diagram](../img/ToulminConstruct.png)

## Contexts and Situtaions with Constructs

Let _context_ be the constrained context of the assessment.
...
_Weight of Evidence_
$$ WOE(H\mathord{:}E) = \log { \frac{\Pr (E |H)}{\Pr (E | \overline H )} }
...
A feature variable is _radical_ if $P(E|H,Z=z_1) \ne P(E|H,Z=z_2)$ or
 $P(E|\overline{H},Z=z_1) \ne P(E|\overline{H},Z=z_2)$

A feature variable is _incidental_ if it is not radical.  It is approximately incidental
if the probability of the $z_2$ value is low.
...
There is dependence on the _base rate_ $P(H)$.
...
Sample of situations in assessment must be a representative sample of the target Context*
in the claim.
...
$T$ -- test result indicates claim holds
$C$ -- claim holds.
...
_Receiver operating characteristic_ or _ROC curve_:
